...
dataset_path <- '/Users/clesio/Downloads/residential_building_dataset.csv'
...
# fazer a carga
residential_building.hex <- 
  h2o.importFile(path = dataset_path,
                 destination_frame = "residential_building.hex")

# Como vamos trabalhar com regressao
# vamos assegurar que a nossa variaveldependente 
# sera numerica
residential_building.hex$out_v1 <- as.numeric(residential_building.hex$out_v1)
residential_building.hex$out_v2 <- as.numeric(residential_building.hex$out_v2)

# Informacoes sobre a base de dados
summary(residential_building.hex)

# Faz a divisao do conjunto de treinamento
# e teste na proporcao 90% para treino
# e 10% para teste
residential_building.split<-
  h2o.splitFrame(data = residential_building.hex,
                 ratios = 0.90,
                 seed = seed)
