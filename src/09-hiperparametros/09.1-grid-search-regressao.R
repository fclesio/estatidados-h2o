  ...
  gbm_params1 <- list(learn_rate = c(0.01, 0.1, 0.2),
                      ntrees = c(50, 200, 500, 1000),
                      stopping_rounds = c(5, 10),
                      stopping_tolerance = c(1e-4, 1e-6),
                      learn_rate_annealing = c(.99, .80)
                      )
  ...
  gbm_gridperf1 <- 
    h2o.getGrid(grid_id = "Grid_GBM_RTMP_sid_b0e5_8_model_R_1588949243046_25951",
                sort_by = "RMSE",
                decreasing = TRUE)
  
  print(gbm_gridperf1)
  
  # Busca o melhor modelo GBM e ordena pelo RMSE
  best_gbm1 <- 
  ...
  model_path_object <- 
    h2o.saveModel(object=best_gbm1,
                  path=artifact_path,
                  force=TRUE
    )
